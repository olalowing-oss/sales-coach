...
export class GatewayServer {
                ...
                try {
                    ...
                    if (!isAuthenticated) {
                        if (message.type === 'connect') {
                            ...
                            const result = await this.handleConnect(ws, connectionId, message);
    ...
    // CONNECTION HANDLER
    // --------------------------------------------------------------------------
    async handleConnect(ws, connectionId, message) {
        ...
        if (userConns.size >= this.config.maxConnectionsPerUser) {
            this.sendError(ws, ErrorCodes.RATE_LIMIT_EXCEEDED, `Maximum ${this.config.maxConnectionsPerUser} connections per user`);
            return { success: false };
        }
    ...
    // MESSAGE ROUTER
    // --------------------------------------------------------------------------
    async handleMessage(connectionId, userId, message) {
        ...
        try {
            switch (message.type) {
                case 'session.start':
                    await this.handleSessionStart(connection, message);
                    break;
                case 'session.transcript':
                    await this.handleSessionTranscript(connection, message);
                    break;
                case 'session.end':
                    await this.handleSessionEnd(connection, message);
                    break;
                case 'tip.dismiss':
                    await this.handleTipDismiss(connection, message);
                    break;
                default:
                    this.sendError(connection.ws, ErrorCodes.INVALID_MESSAGE, `Unknown message type: ${message.type}`);
            }
        }
        catch (error) {
            console.error(`[Gateway] Error handling message:`, error);
            this.sendError(connection.ws, ErrorCodes.INTERNAL_ERROR, error instanceof Error ? error.message : 'Internal server error');
        }
    }
    // --------------------------------------------------------------------------
    // SESSION HANDLERS
    // --------------------------------------------------------------------------
    async handleSessionStart(connection, message) {
    ...
    async handleSessionTranscript(connection, message) {
        const { sessionId, text, isFinal, speaker, confidence } = message.payload;
        // Add segment to session manager
        const segment = await this.sessionManager.addSegment(sessionId, text, speaker, isFinal, confidence);
     ...
     */
    async analyzeAndCoach(connection, sessionId, text, speaker) {
        const session = this.sessionManager.getSession(sessionId);
        if (!session)
            return;
        try {
            // 1. Trigger detection → Coaching tips
            const tips = this.coachingEngine.detectTriggersAndGenerateTips(text, speaker, session);
            ...
            if (speaker !== 'seller') {
                ...
                const sentiment = await this.coachingEngine.analyzeSentiment(sessionId, text, conversationHistory);
                if (sentiment) {
                    this.sendMessage(connection.ws, {
    ...
    async handleSessionEnd(connection, message) {
    ...
    async handleTipDismiss(connection, message) {
