...
export class SessionManager {
    constructor() {
        ...
        this.COMPACTION_INTERVAL = 10; // Compact every 10 messages
        this.KEEP_RECENT = 20; // Keep last 20 segments after compaction
    ...
    createSession(params) {
        ...
        // Save to Supabase
        this.saveSessionToDb(session).catch((err) => console.error('[SessionManager] Failed to save session:', err));
        return session;
    }
    /**
     * Get session by ID
    ...
    getSessionsForUser(userId) {
        ...
        return Array.from(sessionIds)
            ...
            .filter((s) => s !== undefined && s.status === 'active');
    }
    /**
     * End session
     */
     ...
     */
    async addSegment(sessionId, text, speaker, isFinal, confidence) {
        const session = this.sessions.get(sessionId);
        if (!session) {
            throw new Error(`Session not found: ${sessionId}`);
        }
        ...
        if (isFinal) {
            this.saveSegmentToDb(sessionId, segment).catch((err) => console.error('[SessionManager] Failed to save segment:', err));
        }
     ...
     */
    getConversationHistory(sessionId, maxSegments = 20) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return '';
        const parts = [];
    ...
    async compactSession(sessionId) {
        ...
        if (totalSegments <= this.KEEP_RECENT)
            return;
        console.log(`[SessionManager] Compacting session ${sessionId}: ${totalSegments} segments → keep ${this.KEEP_RECENT}`);
        ...
        await this.saveSessionToDb(session);
        console.log(`[SessionManager] Compaction complete: ${oldSegments.length} segments → summary (${summary.length} chars)`);
    }
    /**
     * Summarize segments into text summary
     */
    summarizeSegments(segments, context) {
     ...
     */
    addPainPoint(sessionId, painPoint) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return;
     ...
     */
    addObjection(sessionId, objection) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return;
     ...
     * Add buying signal
     */
    addBuyingSignal(sessionId, signal) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return;
     ...
     */
    updateInterestLevel(sessionId, level) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return;
     ...
     * Update sentiment
     */
    updateSentiment(sessionId, sentiment) {
        const session = this.sessions.get(sessionId);
        if (!session)
            return;
     ...
     */
    async saveSegmentToDb(sessionId, segment) {
        try {
            const segmentData = {
                id: segment.id,
